...
In a Aug 25th 2022 blogpost, Heroku has communicated about thei ["Next Chapter"](https://blog.heroku.com/next-chapter#:~:text=Starting%20October%2026%2C%202022%2C%20we,free%20dynos%20and%20data%20services.).
...
<figure>
    <img src="/images/off-heroku/that-feeling-when-b41e92f404.jpg"
         alt="Heroku meme"/>
    <figcaption>Heroku was a great and cheap PaaS option, if you managed to make it work</figcaption>
</figure>
...
Let's take a simple use case: you have a hooby project which is web app with a database.
Heroku hence runs a server ("dyno") and a Postgres DB ("Postgres add-on") for you.

But Heroku also does more. It is what's called a PaaS (platform as a service): continuous integration with Github, and no headaches integration of your services.
...
But any project running on the cloud has costs: compute and storage.
Storage costs have been driven down "thanks" to AWS S3 and its generous free tier.
(note: for more advanced projects, you might run into high costs due to AWS egress fees, in which case Cloudflare R2 storage wil get you back into low-cost land).
Compute costs however, are not avoidable in most cases.
Developer-friendly and cheap options like Digital Ocean have helped hobby developers being used to pay ~$5 per month for a server and ~$15 per month for a database.
...
If you know your database will stay very small, you can deploy Postgres on your server, side-by-side with your application, saving you the costs of a dedicated DB server.
...
Heroku has an interactive pricing page https://www.heroku.com/pricing to help you estimate (make sure to not choose Free or Hobby).
...
There are a few free options to deploy static websites (look no further than Github Pages).
...
With serverless, and typically AWS Lambda's generous free tier, you can develop the needed backend functionalities "for free".
...
Migrating your application to that architecture might be a fair amount of work, but the cost and PaaS experience might be worth it.
...
Disclaimer: I am a customer, and Supabase is a company from Entrepreneur First, the incubator I was part of in 2018.
...
This is an open-source solution with a lot of promises and a nascent community. The drawback is that you need to bring your own cloud resources (i.e. provision them separately).

### Managed PaaS

If running your own cloud resources is not an option, there are fully managed options.
But keep in mind that running any cloud project has compute + storage costs (see section above).
