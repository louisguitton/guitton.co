...
We'll look at a 10-line code example in LlamaIndex, and see how easy it is to get started.
...
when I worked as Machine Learning Engineer at OneFootball, a football media company from Berlin, Germany.

As a practioner, I remember that time well because it was a time of great change in the NLP field.
...
I was one of the lucky few that could attend the [Spacy IRL 2019](https://irl.spacy.io/2019/) conference in Berlin.
...
In fact, we moved further right and down on his chart to a place deeper than Deep Learning.
...
And in particular, two approaches emerged: Fine Tuning or Retrieval-Augmented Generation (RAG).
...
And according to this Hacker News thread from 2 months ago, [Ask HN: How do I train a custom LLM/ChatGPT on my documents in Dec 2023?](https://news.ycombinator.com/item?id=38759877)
...
**Vector RAG** embeds the query (usually with a smaller model than the LLM; something like FlagEmbeddings) into a vector v<sub>Q</sub>.
...
We note the answer A<sub>SQL</sub> but it's basically data records that result from running Q<sub>SQL</sub> in the database.
The answer A<sub>SQL</sub> as wel as Q<sub>NL</sub> are passed to the LLM to generate A<sub>NL</sub>.

In the case of **Hybrid RAG**, the system uses a combination of the above.
...
There are fine-tuned models specialised in RE like REBEL, but people started using LLMs to extract triples.
...
### 2.2 Implementation in LlamaIndex

With LlamaIndex however, you can build a KG in 10 lines of python.
...
| 2.  | "plays role in" and "is part of cast in" are different relationships that mean the same thing                                                     | Relationships should be consistent or even better matching a provided controlled vocabulary                                                                                                | **Relation Extraction** systems are used to extract standardised relationships             |
| 3.  | triples (Quill, speak uncensored language in, Guardians of the Galaxy) and (James Gunn, could not imagine, Guardians of the Galaxy) are imprecise | If a triple is found, it should resolve to the most important information. In this case (Quill, is present in, Guargians of the Galaxy) or (James Gunn, directed, Guardians of the Galaxy) | Could be mitigated by using a **controlled vocabulary** for relationships                  |
...
Now, with LLMs, we've reached **Abysmal Learning**.
