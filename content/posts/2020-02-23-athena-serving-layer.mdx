...
categories:
  - Data
image: /images/athena-serving-layer/athena.png
---

## Querying Athena in python

Getting data from a datalake or an athena instance is not so hard.
...
Notes that start with _[In theory]_ are things I haven't tried and collected here for completeness.

### Notes

![Optimise Athena](/images/athena-serving-layer/athena-serving-layer.png)
...
1. How to choose between Parquet and ORC: if you don't have many nested data types, Parquet might be unnecessary. There is a tooling affinity for each format though: Hive and ORC, Spark and Parquet, Kafka and Avro... So given that ORC and Parquet are sensibly similar _at my scale_ (note: see point 9 on predicate pushdown), I'd go for Parquet as it's the most convenient when using Spark.
...
1. _[In theory]_ ORC specific notes.
